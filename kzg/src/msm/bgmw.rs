    ...
    > BgmwTable<TFr, TG1, TG1Fp, TG1Affine>
{
    pub fn new(points: &[TG1], matrix: &[Vec<TG1>]) -> Result<Option<Self>, String> {
        ...
        batch_points.try_reserve_exact(matrix.len()).map_err(|_| "BGMW precomputation table is too large".to_owned())?;

        for row in matrix {
            let mut temp_table = Vec::new();
            temp_table.try_reserve_exact(row.len() * batch_h).map_err(|_| "BGMW precomputation table is too large".to_owned())?;

            unsafe { temp_table.set_len(temp_table.capacity()); }
    ...
    pub fn multiply_batch(&self, scalars: &[Vec<TFr>]) -> Vec<TG1> {
        ...
        #[cfg(not(feature = "parallel"))]
        {
            ...
            self.batch_points.iter().zip(scalars).map(|(points, scalars)| Self::multiply_sequential_raw(points, scalars, &mut buckets, window, self.batch_numpoints, self.batch_h)).collect::<Vec<_>>()    
        }
            ...
            for _ in 0..ncpus {
                ...
                pool.joined_execute(move || {
                    ...
                    loop {
                        ...
                        let result = Self::multiply_sequential_raw(&self.batch_points[work], &scalars[work], &mut buckets, window, self.batch_numpoints, self.batch_h);
    ...
    fn multiply_sequential_raw(points: &[TG1Affine], scalars: &[TFr], buckets: &mut [P1XYZZ<TG1Fp>], window: usize, numpoints: usize, h: usize) -> TG1 {
    ...
    pub fn multiply_sequential(&self, scalars: &[TFr]) -> TG1 {
        ...
        Self::multiply_sequential_raw(&self.points, scalars, &mut buckets, window, self.numpoints, self.h)
    }
