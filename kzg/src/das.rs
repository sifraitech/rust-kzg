...
fn deduplicate_with_indices<T: Eq + std::hash::Hash + Clone>(input: &[T]) -> (Vec<T>, Vec<usize>) {
...
pub trait DAS<B: EcBackend> {
    ...
    fn recover_cells_and_kzg_proofs(
        ...
        for (i, &cell_index) in cell_indices.iter().enumerate() {
            ...
            recovered_cells[cell_index * cell_size..(cell_index + 1) * cell_size].clone_from_slice(&cells[i * cell_size..(i + 1) * cell_size]);
        }
    ...
    fn verify_cell_kzg_proof_batch(
        ...
        let (unique_commitments, commitment_indices) = deduplicate_with_indices(
            commitments,
        );
...
fn compute_fk20_proofs<B: EcBackend>(
    ...
    let h_ext_fft = B::G1::g1_lincomb_batch(kzg_settings.get_x_ext_fft_columns(), &coeffs, kzg_settings.get_precomputation())?;
...
fn batch_inverse<B: EcBackend>(v: &mut [B::Fr]) {
    ...
    for (f, s) in v.iter_mut().rev().zip(scratch_pad.iter().rev().skip(1).chain(Some(&B::Fr::one()))) {
